...
class SliderController extends Controller
{
    ...
    public function __construct(){
        ...
        $this->pageLayout = 'admin.pages.slider.';
    ...
    public function index(Builder $builder, Request $request){
        ...
        if (request()->ajax()) {
            ...
            ->editColumn('image', function (Slider $slider){
                if(!$slider->image){
                    return '<img class="img-thumbnail" src="' . asset('storage/slider/default.png').'" style="width:60px; height:48px;">';
                }else{
                    return '<img class="img-thumbnail" src="' . asset('storage/slider' . '/' . $slider->image) . '" style="width:60px; height:48px;">';
                }
            })
            ->editColumn('action', function (Slider $slider) {
                $action  = '';
                $action .= '<a class="btn btn-warning btn-circle btn-sm" href='.route('admin.slider.edit',[$slider->id]).'><i class="fa fa-pencil" data-toggle="tooltip" title="Edit"></i></a>';
            ...
            ->rawColumns(['action','sub_title','description','status','image'])
            ->make(true);
        }
