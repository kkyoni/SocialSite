...
class BlogController extends Controller{
    ...
    public function index(Builder $builder, Request $request){
        ...
        if (request()->ajax()) {
            ...
            ->editColumn('action', function (Blog $blog) {
                ...
                $action .= '<a class="btn btn-warning btn-circle btn-sm" href='.route('admin.blog.blog_comment',[$blog->id]).'><i class="fa fa-star" data-toggle="tooltip" title="Product Comment"></i></a>';
                return $action;
            })
    ...
    public function blog_comment(Request $request,$id){
        $blog = Blog::where('id',$id)->first();
        $count_comment = Comment::where('only_id',$id)->where('type','blog')->count();
        return view($this->pageLayout.'blog_comment',compact('blog','count_comment'));
    }
    function loadmorecomment(Request $request){
        $site_name = Setting::where('code','project_title')->first();
        if($request->ajax()){
            if($request->id > 0){
                $data = Comment::with('user')->where('only_id',$request->blog_id)->where('type','blog')->where('id', '<', $request->id)->orderBy('id','desc')->limit(1)->get();
            }else{
                $data = Comment::with('user')->where('only_id',$request->blog_id)->where('type','blog')->orderBy('id','desc')->limit(1)->get();
            }
            $output = '';
            $last_id = '';
            if(!$data->isEmpty()){
                foreach($data as $row){
                    ...
                    <img alt="image" class="rounded-circle" src="/storage/avatar/'.$row->user->avatar.'">
                    </a>
                    <div class="media-body ">
                    <strong>'.$row->user->name.'</strong> Blog Comment on <strong>'.$site_name->value.'</strong> site. <br>
                    <small class="text-muted">'.Helper::DateFormate($row->created_at).'</small>
                    <div class="well">'.$row->comment.'</div>
                    </div>
                    </div>';
                    $last_id = $row->id;
                }
                $output .= '
                <button class="btn btn-primary btn-block m" name="load_more_button" data-id="'.$last_id.'" id="load_more_button"><i class="fa fa-arrow-down"></i> Show More</button>';
            }else{
                $output .= '
                <button class="btn btn-primary btn-block m" name="load_more_button" disabled><i class="fa fa-arrow-down"></i> No Data Found</button>';
            }
            echo $output;
        }
    }
